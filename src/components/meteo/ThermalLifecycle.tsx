...
const ETAPAS = [
  ...
  },
  {
    ...
    texto: "A bolha se desprende do solo — muitas vezes por um gatilho, como um vento local ou um desnível.",
  },
  {
    nome: "4. Ascensão",
  ...
  },
  {
    ...
    texto: "Várias bolhas formam uma coluna ascendente contínua — a térmica que o piloto usa para ganhar altitude.",
  },
  {
    nome: "6. Formação de nuvem",
    texto: "Lá em cima, o ar esfria e a umidade condensa: nasce um cumulus, que marca o topo da térmica.",
  },
  {
    nome: "7. Enfraquecimento",
    texto: "Sem mais aquecimento (ou sob a sombra da própria nuvem), a térmica perde força e se dissolve.",
  },
];
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      if (e === 0) {
        ...
        ctx.strokeStyle = "rgba(240,140,40,0.7)";
        ctx.lineWidth = 2;
        for (let i = -2; i <= 2; i++) {
          ctx.beginPath();
      ...
      if (e >= 1 && e <= 4) {
        ...
        ctx.fillStyle = "rgba(240,140,40,0.5)";
        ctx.beginPath();
        ctx.arc(cx + Math.sin(t * 2) * 4, by, br, 0, Math.PI * 2);
        ctx.fill();
        // partículas internas
        ctx.fillStyle = "rgba(240,140,40,0.85)";
        for (let i = 0; i < 10; i++) {
          const a = t * 1.5 + i * 0.63;
          ctx.beginPath();
          ctx.arc(cx + Math.cos(a) * br * 0.6, by + Math.sin(a * 1.3) * br * 0.6, 2.4, 0, Math.PI * 2);
          ctx.fill();
        }
        if (e === 1) {
      ...
      if (e === 4) {
        // coluna organizada
        ctx.strokeStyle = "rgba(240,140,40,0.5)";
        ctx.lineWidth = 26;
        ctx.beginPath();
      ...
      if (e === 6) {
        ...
        ctx.strokeStyle = "rgba(240,140,40,0.22)";
        ctx.lineWidth = 30;
        ctx.beginPath();
  ...
  return (
    <div>
      <canvas
        ref={canvasRef}
        role="img"
        aria-label={`Etapas do ciclo de vida de uma térmica. Etapa atual: ${atual.nome}`}
