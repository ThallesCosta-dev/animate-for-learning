...
const NUVENS: Nuvem[] = [
  {
    ...
    observar: "Cumulus pequenos e espaçados marcam térmicas usáveis. Observe se estão crescendo rápido demais.",
    riscos: "Crescimento vertical rápido pode evoluir para cumulonimbus. Base muito escura indica nuvem madura e sugada forte embaixo.",
    perigo: 1,
  },
  {
    id: "stratus",
    ...
    riscos: "Teto baixo pode esconder o relevo e reduzir a visibilidade. Voo geralmente fraco e sem sustentação.",
    perigo: 2,
  },
  {
    id: "cumulonimbus",
    ...
    observar: "Torre escura crescendo rápido, topo em bigorna, base escura, vento mudando de direção e rajadas.",
    riscos: "PERIGO EXTREMO: correntes ascendentes e descendentes violentas, granizo, raios e rajadas. Não voar — se estiver no ar, pousar o quanto antes em local seguro.",
    perigo: 3,
  },
];
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      <div className="rounded-xl border border-border bg-skyblue/25 p-6">
        <svg viewBox="0 0 340 220" className="w-full" role="group" aria-label="Tipos de nuvem — clique para saber mais">
          {/* Cumulus */}
          <g
          ...
          >
            <g fill="#f4f7fb" stroke={ativa.id === "cumulus" ? "var(--primary)" : "#9db2c8"} strokeWidth={ativa.id === "cumulus" ? 3 : 1.5}>
            ...
            <text x={95} y={162} textAnchor="middle" fontSize={13} fontWeight="bold" fill="#33475e">Cumulus</text>
          </g>
          {/* Stratus */}
          <g
            className="cursor-pointer"
          ...
          >
            <g fill="#c9d2dc" stroke={ativa.id === "stratus" ? "var(--primary)" : "#8a99ab"} strokeWidth={ativa.id === "stratus" ? 3 : 1.5}>
              <ellipse cx={245} cy={70} rx={62} ry={14} />
              <ellipse cx={245} cy={86} rx={70} ry={12} />
            </g>
            <text x={245} y={120} textAnchor="middle" fontSize={13} fontWeight="bold" fill="#33475e">Stratus</text>
          </g>
          {/* Cumulonimbus */}
          <g
            className="cursor-pointer"
          ...
          >
            <g fill="#7d8ea3" stroke={ativa.id === "cumulonimbus" ? "var(--destructive)" : "#5c6b7d"} strokeWidth={ativa.id === "cumulonimbus" ? 3 : 1.5}>
              <ellipse cx={235} cy={185} rx={52} ry={16} />
              <circle cx={235} cy={160} r={24} />
              <circle cx={222} cy={140} r={18} />
              <ellipse cx={235} cy={124} rx={40} ry={10} />
            </g>
            <text x={235} y={216} textAnchor="middle" fontSize={13} fontWeight="bold" fill="#8a2020">Cumulonimbus ⚠</text>
          </g>
        </svg>
      </div>
